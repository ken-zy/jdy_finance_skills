...
2. **SEARCH FOR LATEST** - Use MCP tools or web search to find the most recent earnings
...
- [ ] **Actively searched** using MCP tools or "latest earnings" web search
...
- **10-Q or 10-K filing** - On SEC EDGAR (sec.gov/cgi-bin/browse-edgar)
...
- **Earnings call transcript**
  - **MCP first**: Query **alpha-vantage** for earnings call transcripts
  - **Web fallback**:
    - Search for: "[Company] latest earnings call transcript" or "[Company] Q[X] [Year] earnings call transcript"
    - Sources: Seeking Alpha (seekingalpha.com/earnings/transcripts), company IR site, AlphaStreet, Motley Fool
    - Use **Chrome CDP** if Seeking Alpha requires login for full transcript
  - **CRITICAL DATE CHECK**:
...
- [ ] **SEARCHED** using MCP tools or "latest earnings" web search (not assumed based on current date)
...
### Step 14: Optional - Update Financial Model
...
**Note**: For earnings updates, a full model file is OPTIONAL (not required like in initiation reports). The report is the primary deliverable.

If creating model, include:
- Quarterly model tab
...
1. **Report file**: `[Company]_Q[X]_[Year]_Earnings_Update`
2. **Chart files**: All PNG/JPG charts (for reference)
3. **Optional**: Updated financial model if maintained
